...
describe("normalizeNewsMarkdown", () => {
  it("converts Territorium pull request URLs to short markdown links", () => {
    const input =
      "Fix attack logic in https://github.com/BroussenDev/Territorium/pull/1234";

    const result = normalizeNewsMarkdown(input);

    expect(result).toContain(
      "[#1234](https://github.com/BroussenDev/Territorium/pull/1234)",
    );
  });

  it("converts Territorium compare URLs to markdown links", () => {
    const input =
      "Full Changelog: https://github.com/BroussenDev/Territorium/compare/v1.0.0...v1.1.0";

    const result = normalizeNewsMarkdown(input);

    expect(result).toContain(
      "[v1.0.0...v1.1.0](https://github.com/BroussenDev/Territorium/compare/v1.0.0...v1.1.0)",
    );
  });
