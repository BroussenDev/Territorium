const GITHUB_PR_URL_REGEX =
  /(?<!\()\bhttps:\/\/github\.com\/BroussenDev\/Territorium\/pull\/(\d+)\b/g;
const GITHUB_COMPARE_URL_REGEX =
  /(?<!\()\bhttps:\/\/github\.com\/BroussenDev\/Territorium\/compare\/([\w.-]+)\b/g;
  ...
  return (
    markdown
      ...
      .replace(
        GITHUB_PR_URL_REGEX,
        (_match, prNumber) =>
          `[#${prNumber}](https://github.com/BroussenDev/Territorium/pull/${prNumber})`,
      )
      .replace(
        GITHUB_COMPARE_URL_REGEX,
        (_match, comparison) =>
          `[${comparison}](https://github.com/BroussenDev/Territorium/compare/${comparison})`,
      )
      .replace(
