        ...
        >
          ${this.player.clanTag
            ? html`<div
                ...
                class="inline-flex min-w-0 max-w-[40%] shrink-0 rounded-md border border-brand/20 bg-brand/10 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-brand-light/85"
              >
  ...
  private renderScoreBar(formattedScore: string) {
    ...
    return html`
      <div class="w-full">
        ...
        >
          <div
            class="h-full rounded-full bg-gradient-to-r from-brand/65 to-brand-light shadow-[0_0_10px_rgba(63,169,245,0.2)] w-(--width)"
            style="--width: ${width}%;"
          ></div>
        </div>
      </div>
  ...
  private renderIcon() {
    ...
    return html`
      <div
        ...
        class="flex size-10 items-center justify-center rounded-xl border border-brand/15 bg-gradient-to-br from-brand/20 to-white/[0.04] text-xs font-bold uppercase tracking-wide text-brand-light/80"
        aria-hidden="true"
      >
        ${this.initials()}
      </div>
